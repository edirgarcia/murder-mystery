...
import { getGameInfo, startGame, beginGame, endGame, getResults } from "../api/http";
import type { Difficulty, WSEvent, LeaderboardEntry, ClueInfo } from "../types/game";
    ...
    (event: WSEvent) => {
      switch (event.event) {
        ...
        case "game_over": {
          ...
          // Fetch full results for murder clues
          if (code) {
            getResults(code).then((results) => {
              setMurderClues(results.murder_clues);
              setMurderWeaponLocal(results.murder_weapon);
